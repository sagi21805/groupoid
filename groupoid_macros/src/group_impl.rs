...
use syn::{
    AngleBracketedGenericArguments, Ident, ItemImpl, PathArguments,
...
impl<'ast> GroupImpl<'ast> {
    ...
    pub fn create_group_impl(&self) -> syn::Result<TokenStream> {
        ...
        let (trait_path, _) =
            modified.trait_.as_mut().ok_or(syn::Error::new_spanned(
                self.inner_impl,
        ...
        let last: &mut PathSegment =
            trait_path.segments.last_mut().unwrap();
